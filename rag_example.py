...
# For this example, we'll use the hardcoded key from upload_to_atlas.py for simplicity,
# but environment variables are recommended for production.
...
# Replace with your actual Atlas project ID and map ID
# You can find these in the URL of your Atlas project:
# https://atlas.nomic.ai/data/<organization_name>/<project_id>/map/<map_id>
ATLAS_ORGANIZATION_NAME = "talon-pro-se"
ATLAS_PROJECT_ID = "stephen-boerner-divorce-case-data-test-upload"
ATLAS_MAP_ID = "0197c9a8-2dc7-f538-1298-06c2dae141f8" # This was printed in the previous successful upload
...
def retrieve_relevant_data(query, k=3):
    ...
    try:
        ...
        # Get the project (dataset) and map
        project = atlas.get_project(
            name=ATLAS_PROJECT_ID,
            organization_name=ATLAS_ORGANIZATION_NAME
        )
        
        map_obj = project.get_map(map_id=ATLAS_MAP_ID) # Access the map directly

        # Perform vector search
        # The query will be embedded and then used to find similar data points
        results = map_obj.vector_search(queries=[query], k=k)
        
        retrieved_data = []
        for idx, score in zip(results['neighbors'][0], results['distances'][0]):
            # Retrieve the original data point from the dataset
            # This might require fetching the data from Atlas or having it locally
            # For this example, we'll assume we can get the data by index from the original nomic_atlas_data.json
...
if __name__ == "__main__":
    ...
    context_data = retrieve_relevant_data(user_query, k=5) # Retrieve top 5 relevant data points
